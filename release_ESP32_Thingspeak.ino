...
String apiKey = "X6X4GKTLUHZAB5IR"; 
...
void sendToThingSpeak() 
{
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    String url = String(server) + "?api_key=" + apiKey +
                 "&field1=" + String(tempAvg, 2) +
                 "&field2=" + String(gndHumAvg, 2) +
                 "&field3=" + String(digitalRead(PELTIER2_HOT_PIN)) +
                 "&field4=" + String(digitalRead(WATER_PUMP_PIN)) +
                 "&field5=" + String(digitalRead(RGB_STRIPE_PIN)) +
                 "&field6=" + String(val, 2) +
                 "&field7=" + String(c, 2) +
                 "&field8=" + String(DAC_INTERVAL);
    ...
    {
      Serial.println("Error al conectar a ThingSpeak: " + String(httpCode));
    }
    http.end();
  } 
  else
...
void loop() 
{
  ...
  /* Send data to thingspeak each 15 sec */
